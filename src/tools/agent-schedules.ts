...
export function register(server: McpServer) {
  ...
  server.tool(
    ...
    "Create a recurring Rolli Agent report that runs on a cron-like cadence (daily, weekly, monthly, or one-time) with optional email and Slack notifications.",
    {
      ...
      available_credits: z.number().int().min(1).max(4).optional().describe("Reasoning effort for each run: 1=low, 2=medium, 3=high, 4=max (default)"),
      ...
      max_post: z.number().int().positive().optional().describe("Posts per platform per run (default: 100)"),
